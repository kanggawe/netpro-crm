...
// Resolve Customer ID supporting Direct ID, Masked ID, or Encrypted Token
$custId = 0;
if (!empty($_GET['token'])) {
    $custId = intval(url_decrypt($_GET['token'], 'customer') ?: url_decrypt($_GET['token']));
} elseif (!empty($_GET['ref'])) {
    $custId = intval(url_decrypt($_GET['ref'], 'customer') ?: url_decrypt($_GET['ref']));
} elseif (!empty($_GET['mid'])) {
    $custId = unmask_id($_GET['mid']);
} elseif (!empty($_GET['id'])) {
    $rawId = $_GET['id'];
    $custId = is_numeric($rawId) ? intval($rawId) : (unmask_id($rawId) ?: intval(url_decrypt($rawId, 'customer')));
}
