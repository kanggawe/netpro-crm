...
class Package {
    ...
    public static function update($id, $data) {
        global $pdo;
        $stmt = $pdo->prepare("UPDATE packages SET name = ?, speed_mbps = ?, price = ?, default_ppn_mode = ?, category = ? WHERE id = ?");
        return $stmt->execute([
            $data['name'],
            intval($data['speed_mbps'] ?? 0),
            floatval($data['price'] ?? 0),
            $data['default_ppn_mode'] ?? 'include',
            $data['category'] ?? 'home',
            intval($id)
        ]);
    }
...
class Addon {
    ...
    public static function update($id, $data) {
        global $pdo;
        $stmt = $pdo->prepare("UPDATE addons SET name = ?, category = ?, price = ?, description = ? WHERE id = ?");
        return $stmt->execute([$data['name'], $data['category'] ?? 'ADDON PRO', floatval($data['price'] ?? 0), $data['description'], intval($id)]);
    }
    public static function delete($id) {
        global $pdo;
...
class Promo {
    ...
    public static function update($id, $data) {
        global $pdo;
        $stmt = $pdo->prepare("UPDATE promos SET code = ?, title = ?, discount_amount = ?, quota = ?, valid_until = ?, status = ? WHERE id = ?");
        return $stmt->execute([
            strtoupper($data['code']),
            $data['title'],
            floatval($data['discount_amount'] ?? 0),
            intval($data['quota'] ?? 100),
            $data['valid_until'] ?? date('Y-12-31'),
            $data['status'] ?? 'AKTIF',
            intval($id)
        ]);
    }
...
class User {
    ...
    public static function updateProfile($id, $data) {
        global $pdo;
        // Auto-migration for extra profile fields
        try { $pdo->exec("ALTER TABLE users ADD COLUMN avatar TEXT"); } catch (Throwable $e) {}
        try { $pdo->exec("ALTER TABLE users ADD COLUMN bio TEXT"); } catch (Throwable $e) {}
        try { $pdo->exec("ALTER TABLE users ADD COLUMN nik VARCHAR(50)"); } catch (Throwable $e) {}
        try { $pdo->exec("ALTER TABLE users ADD COLUMN telegram_id VARCHAR(50)"); } catch (Throwable $e) {}
        try { $pdo->exec("ALTER TABLE users ADD COLUMN address TEXT"); } catch (Throwable $e) {}

        $avatar = !empty($data['avatar']) ? trim($data['avatar']) : 'assets/images/avatar-admin.svg';
        $bio = $data['bio'] ?? '';
        $nik = $data['nik'] ?? '';
        $telegram = $data['telegram_id'] ?? '';
        $address = $data['address'] ?? '';

        $stmt = $pdo->prepare("UPDATE users SET full_name = ?, email = ?, phone = ?, division = ?, role = ?, avatar = ?, bio = ?, nik = ?, telegram_id = ?, address = ? WHERE id = ?");
        return $stmt->execute([
            $data['full_name'] ?? 'Super Admin',
            $data['email'] ?? 'admin@netpro.id',
