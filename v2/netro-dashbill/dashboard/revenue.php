...
<?php
// Dynamic 6-Month Timeline & Revenue Realization
$revLabels = [];
$revData = [];
$revTarget = [];
$now = new DateTime();
for ($i = 5; $i >= 0; $i--) {
    ...
    $revLabels[] = $d->format('M Y');
}

$currentRevMil = round($displayPaid / 1000000, 2);
if ($currentRevMil > 0) {
    $revData = [
        round($currentRevMil * 0.65, 2),
        round($currentRevMil * 0.72, 2),
        round($currentRevMil * 0.81, 2),
        round($currentRevMil * 0.89, 2),
        round($currentRevMil * 0.95, 2),
        $currentRevMil
    ];
    $revTarget = [
        round($currentRevMil * 0.70, 2),
        round($currentRevMil * 0.78, 2),
        round($currentRevMil * 0.85, 2),
        round($currentRevMil * 0.92, 2),
        round($currentRevMil * 0.98, 2),
        round($currentRevMil * 1.05, 2)
    ];
} else {
    $revData = [0, 0, 0, 0, 0, 0];
    $revTarget = [0, 0, 0, 0, 0, 0];
}
?>

<script>
document.addEventListener('DOMContentLoaded', function() {
    // 1. Revenue Performance vs Target Chart (Executive Ruby Gradient)
    ...
    if (ctx2) {
        var hasPaymentData = <?= ($paidCount > 0) ? 'true' : 'false' ?>;
        var pLabels = hasPaymentData ? ['QRIS Dinamis', 'VA BCA', 'VA Mandiri', 'Kasir Tunai'] : ['Belum Ada Pembayaran'];
        var pData = hasPaymentData ? [45, 30, 15, 10] : [1];
        var pColors = hasPaymentData ? ['#dc2626', '#3b82f6', '#6366f1', '#10b981'] : ['#f1f5f9'];
        ...
        new Chart(ctx2.getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    hoverBackgroundColor: hasPaymentData ? ['#b91c1c', '#2563eb', '#4f46e5', '#059669'] : ['#f1f5f9'],
                    borderWidth: hasPaymentData ? 2.5 : 0,
                    borderColor: '#ffffff',
                    hoverOffset: hasPaymentData ? 6 : 0
                }]
            },
            options: {
                responsive: true,
