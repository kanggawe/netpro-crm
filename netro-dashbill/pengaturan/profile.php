    ...
    <div class="bg-gradient-to-r from-brand-950 via-slate-950 to-brand-950 text-white p-6 sm:p-8 rounded-3xl shadow-xl border border-brand-900/40 relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
            <div class="flex items-center gap-5">
                <div class="relative">
                    <div class="w-20 h-20 rounded-2xl bg-gradient-to-tr from-brand-600 to-rose-600 flex items-center justify-center font-bold text-white text-3xl shadow-xl ring-2 ring-white/20">
        ...
        <div class="lg:col-span-7 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-5">
            <div class="border-b border-slate-100 pb-3 flex justify-between items-center">
                <h3 class="font-bold text-slate-900 text-sm flex items-center gap-2">
                    <i class="fa-solid fa-id-card text-blue-600"></i> Informasi Profil Akun
                </h3>
                <span class="text-slate-400 text-[11px]">ID User: #<?= $user['id'] ?></span>
            </div>
            ...
            <form action="<?= base_url('api/handler.php') ?>" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="username" readonly value="<?= htmlspecialchars($user['username']) ?>" class="w-full bg-slate-100 border border-slate-200 rounded-lg p-2.5 font-mono font-bold text-slate-500 cursor-not-allowed">
                    </div>
                    <div>
                        <label class="font-semibold text-slate-700 block mb-1">Nama Lengkap</label>
                        <input type="text" name="full_name" required value="<?= htmlspecialchars($user['full_name']) ?>" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 font-bold text-slate-900">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="email" name="email" required value="<?= htmlspecialchars($user['email']) ?>" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 font-medium">
                    </div>
                    <div>
                        <label class="font-semibold text-slate-700 block mb-1">No. WhatsApp / Telepon</label>
                        <input type="text" name="phone" value="<?= htmlspecialchars($user['phone'] ?? '0812-9876-5432') ?>" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 font-mono font-bold text-emerald-600">
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="font-semibold text-slate-700 block mb-1">Divisi / Unit Kerja</label>
                        <input type="text" name="division" value="<?= htmlspecialchars($user['division'] ?? 'NOC & Core Infrastructure') ?>" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 font-bold text-slate-800">
                    </div>
                    <div>
                        <label class="font-semibold text-slate-700 block mb-1">Role Hak Akses (RBAC)</label>
                        <select name="role" class="w-full bg-slate-50 border border-slate-200 rounded-lg p-2.5 font-bold text-blue-600">
                            <option <?= strpos($user['role'], 'Super') !== false ? 'selected' : '' ?>>Super Admin (All Access)</option>
                            <option <?= strpos($user['role'], 'Finance') !== false ? 'selected' : '' ?>>Finance & Billing Manager</option>
                            <option <?= strpos($user['role'], 'NOC') !== false ? 'selected' : '' ?>>NOC & Network Engineer</option>
                            <option <?= strpos($user['role'], 'Support') !== false || strpos($user['role'], 'Customer') !== false ? 'selected' : '' ?>>Customer Support & Helpdesk</option>
                            <option <?= strpos($user['role'], 'Teknisi') !== false ? 'selected' : '' ?>>Teknisi Lapangan (Field Ops)</option>
                            <option <?= strpos($user['role'], 'Sales') !== false ? 'selected' : '' ?>>Sales & Marketing Executive</option>
                        </select>
                    </div>
                </div>

                <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-xl shadow-md transition flex items-center justify-center gap-2">
